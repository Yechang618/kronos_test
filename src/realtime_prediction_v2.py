...
"""
实时Kronos预测系统 v2.1 (修复版)
- 修复Binance符号格式问题：自动将BTC→BTCUSDT
- 修正合约API路径：/fapi/v1/depth (非/api/v3/depth)
- 每分钟采集Binance订单簿和资金费率数据
- 每10分钟计算1次K线（基于该10分钟窗口内所有数据点）
...
# 配置参数
CONFIG = Config()
# 自动标准化符号：确保所有symbol以USDT结尾
SYMBOL_LIST = [s.upper() + 'USDT' if not s.upper().endswith('USDT') else s.upper() for s in CONFIG.symbol_list]
MAX_ROWS = 144
PRED_LENGTH = 6
...
class BinanceDataFetcher:
    ...
    def _standardize_symbol(self, symbol: str) -> str:
        """标准化交易对符号为Binance格式（确保包含USDT）"""
        symbol = symbol.upper().strip()
        if not symbol.endswith('USDT'):
            # 检查是否已是有效交易对（如BTCBUSD）
    ...
    def fetch_orderbook(self, symbol: str, is_swap: bool = False) -> Optional[Dict]:
        """获取订单簿数据（第一档）- 修复合约API路径"""
        try:
            symbol_std = self._standardize_symbol(symbol)
            ...
            print(f"[ERROR] {api_type}订单簿获取失败 {symbol} → {symbol_std}: {str(e)}")
            if "400" in str(e):
                print(f"  └─ 可能原因: 无效的交易对符号 '{symbol_std}'，请检查Binance是否支持该交易对")
            elif "403" in str(e):
    ...
    def fetch_funding_rate(self, symbol: str) -> float:
        ...
        try:
            ...
            params = {"symbol": symbol_std, "limit": 1}
            ...
            symbol_std = self._standardize_symbol(symbol)
            print(f"[ERROR] 资金费率获取失败 {symbol} → {symbol_std}: {str(e)}")
            return 0.0
    ...
    def fetch_index_price(self, symbol: str) -> float:
        ...
        try:
            ...
            if self.test_mode:
                spot_ob = self.fetch_orderbook(symbol_std, is_swap=False)
                if spot_ob:
            ...
            symbol_std = self._standardize_symbol(symbol)
            print(f"[WARNING] 指数价格获取失败 {symbol} → {symbol_std}, 使用现货中间价替代: {str(e)}")
            spot_ob = self.fetch_orderbook(symbol_std, is_swap=False)
            if spot_ob:
                return (spot_ob['bid0_price'] + spot_ob['ask0_price']) / 2
            return 10000.0
...
class RealtimeKlineManager:
    ...
    def __init__(self, max_rows: int = MAX_ROWS, save_dir: Path = KLINE_SAVE_DIR):
    ...
    def compute_kline_from_buffer(self, symbol: str, window_start: datetime, window_end: datetime) -> Optional[Dict]:
        ...
        try:
            ...
            # 计算K线各字段（符合新需求）
            open_price = window_data[0]['basis_mid']          # 窗口第一个点的basis_mid
            close_price = window_data[-1]['basis_mid']        # 窗口最后一个点的basis_mid
    ...
    def load_klines_from_disk(self) -> None:
        ...
        print(f"[INIT] 尝试从 {self.save_dir} 加载历史K线数据...")
        ...
        if loaded_count > 0:
            print(f"[INIT] 成功加载 {loaded_count} 个币种的历史K线数据")
        else:
...
def validate_binance_symbols(symbol_list: List[str]) -> bool:
    """启动前验证Binance交易对有效性"""
    print(f"[INIT] 验证Binance交易对有效性: {symbol_list}")
    fetcher = BinanceDataFetcher(test_mode=False)
    valid_count = 0
    ...
    for symbol in symbol_list:
        ...
        if spot_ob and swap_ob:
            print(f"  ✓ {symbol}: 现货和合约数据均可获取")
            valid_count += 1
        else:
            print(f"  ✗ {symbol}: 数据获取失败（请检查Binance是否支持该交易对）")
    
    if valid_count == 0:
        print("\n[CRITICAL] 所有交易对验证失败！程序将退出。")
        print("常见原因:")
        print("  1. 交易对在Binance不存在（如TAO可能需要使用TAOBTC）")
        print("  2. 网络问题或API限制")
        print("  3. 配置中的符号格式错误")
        return False
...
def main(test_mode: bool = False):
    """主循环"""
    print(f"{'='*70}")
    print(f"Kronos实时预测系统 v2.1 (修复版) 启动")
    print(f"原始配置符号: {CONFIG.symbol_list}")
    print(f"标准化后符号: {SYMBOL_LIST}")
    print(f"K线保留: 最多{MAX_ROWS}个10分钟K线")
    print(f"数据采集: 每分钟整点采集原始数据")
    print(f"K线计算: 每10分钟整点计算K线（open=首basis_mid, close=尾basis_mid, high=max(basis_bid), low=min(basis_ask)）")
    print(f"数据持久化: K线自动保存到 {KLINE_SAVE_DIR}")
    print(f"测试模式: {'ENABLED' if test_mode else 'DISABLED'}")
    print(f"{'='*70}\n")
    
    # 非测试模式下验证交易对
    if not test_mode:
        if not validate_binance_symbols(SYMBOL_LIST):
            print("[EXIT] 交易对验证失败，退出程序")
            return
    
    # 初始化组件
    fetcher = BinanceDataFetcher(test_mode=test_mode)
    kline_manager = RealtimeKlineManager(max_rows=MAX_ROWS, save_dir=KLINE_SAVE_DIR)
    
    # 为每个symbol初始化独立的DynamicSignalGenerator
    ...
    while True:
        try:
            ...
            if should_collect_data(current_time) and current_minute != last_data_collect_minute:
                print(f"\n[DATA] [{current_time.strftime('%Y-%m-%d %H:%M:%S')}] 采集原始数据...")
            ...
            if should_trigger_full_prediction(current_time) and current_minute != last_full_pred_minute:
                ...
                if result_dict:
                    ...
                    result_path = Path("./data/predictions") / f"full_pred_{timestamp_str}.json"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Kronos实时预测系统 v2.1 (修复版)")

...
# def process_tick_df(df, prefix):
#     df_out = pd.DataFrame(index=df.index)
#     tick_cols = ["index_price", "mark_price", "funding_rate"]
#     for col in tick_cols:
#         if col in df.columns:
#             df_out[f"{prefix}_{col}"] = df[col]
#     return df_out
...
def load_and_process_data(symbol, date_range, load_dir, output_dir):
    ...
    for single_date in date_range:
        ...
        ftick_path = load_dir / f"ftick/binance-futures_derivative_ticker_{date_str}_{pair}.csv.gz"
        ...
        try:
            ...
            tick_df = pd.read_csv(ftick_path)
            tick_df.index = parse_timestamp_series(tick_df["timestamp"])
            tick_df = tick_df.sort_index()
            if tick_df.index.duplicated().any():
                tick_df = tick_df[~tick_df.index.duplicated(keep='last')]
            tick_df = tick_df[["index_price", "mark_price", "funding_rate"]]
            tick_res = tick_df.reindex(full_index, method='pad')       
            

            df_day = pd.concat([spot_res, swap_res, tick_res], axis=1)
            df_day = calculate_basis_metrics(df_day)
            df_day.index.name = "timestamp"

            if df_day["spot_bid0_price"].first_valid_index() is not None:
                # 保存当日数据
                daily_file = output_dir / f"backtest_fdr_{symbol}_{date_str}.csv"
...
def main():
    ...
    selected_symbols = ["AVAX","FET", "KAITO","LINK", "TAO", "TRX", "XRP", "ZEC"]   
    for symbol in selected_symbols:
        ...
        for start_str, end_str in time_periods:
            ...
            for date in date_range:
                ...
                file_path = output_dir / f"backtest_fdr_{symbol}_{date_str}.csv"
                if file_path.exists():
            ...
            if not combined_df.empty:
                ...
                csv_file = output_dir / f"backtest_fdr_{symbol}_{start_time}_{end_time}.csv"

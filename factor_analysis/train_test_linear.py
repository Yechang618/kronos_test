...
增强版：添加测试集 y_true vs y_pred 可视化功能
"""
import pandas as pd
import numpy as np
...
class ImportanceVisualizer:
    ...
    def plot_top_factors(self, importance_df, symbol):
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / f'{symbol}_top_factors.png', dpi=150, bbox_inches='tight')
        plt.close()
        print(f"  📈 保存 Top 因子图：{symbol}_top_factors.png")
    ...
    def plot_significance_table(self, importance_df, symbol):
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / f'{symbol}_significance_table.png', dpi=150, bbox_inches='tight')
        plt.close()
        print(f"  📈 保存显著性表格：{symbol}_significance_table.png")
    ...
    def plot_coefficient_distribution(self, importance_df, symbol):
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / f'{symbol}_coefficient_distribution.png', dpi=150, bbox_inches='tight')
        plt.close()
        print(f"  📈 保存系数分布图：{symbol}_coefficient_distribution.png")
    
    # ============================
    # ✅ 新增：测试集 y_true vs y_pred 可视化方法
    # ============================
    ...
    def plot_true_vs_pred_scatter(self, y_true: np.ndarray, y_pred: np.ndarray, 
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / f'{symbol}_true_vs_pred_scatter.png', dpi=150, bbox_inches='tight')
        plt.close()
        print(f"  📈 保存散点图：{symbol}_true_vs_pred_scatter.png")
    ...
    def plot_true_pred_timeseries(self, y_true: np.ndarray, y_pred: np.ndarray, 
        ...
        axes[0].plot(y_true_plot, label='True', linewidth=1, alpha=0.8, color='blue')
        axes[0].plot(y_pred_plot, label='Predicted', linewidth=1, alpha=0.8, color='orange')        
        axes[0].set_ylabel('Value')
        # axes[0].set_yscale('log')
        axes[0].set_title(f'{symbol}: True vs Predicted Time Series' if symbol else 'True vs Predicted Time Series')
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / f'{symbol}_true_pred_timeseries.png' if symbol else 'true_pred_timeseries.png', 
                   dpi=150, bbox_inches='tight')
        plt.close()
        print(f"  📈 保存时间序列对比图：{symbol}_true_pred_timeseries.png")
    ...
    def plot_residual_analysis(self, y_true: np.ndarray, y_pred: np.ndarray, symbol: str):
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / f'{symbol}_residual_analysis.png', dpi=150, bbox_inches='tight')
        plt.close()
        print(f"  📈 保存残差分析图：{symbol}_residual_analysis.png")
    ...
    def plot_cumulative_returns(self, y_true: np.ndarray, y_pred: np.ndarray, symbol: str):
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / f'{symbol}_cumulative_returns.png', dpi=150, bbox_inches='tight')
        plt.close()
        print(f"  📈 保存累积收益图：{symbol}_cumulative_returns.png")
    ...
    def plot_quantile_returns(self, quantile_returns: dict, symbol: str):
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / f'{symbol}_quantile_returns.png', 
                dpi=150, bbox_inches='tight')
        plt.close()
    ...
    def plot_prediction_distribution(self, y_true: np.ndarray, y_pred: np.ndarray, symbol: str):
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / f'{symbol}_prediction_distribution.png', dpi=150, bbox_inches='tight')
        plt.close()
        print(f"  📈 保存分布对比图：{symbol}_prediction_distribution.png")
    
    def generate_all_test_plots(self, symbol: str, test_results: dict):
        """生成全部测试可视化"""
        print("  🎨 生成测试集可视化图表...")
...
def train_test_symbol(symbol: str, config: Config) -> dict:
    ...
    # 4. 数据分割
    X_temp, X_test, y_reg_temp, y_reg_test, indices_temp, indices_test = train_test_split(
        X, y_reg, indices, test_size=config.TEST_RATIO, random_state=config.RANDOM_SEED
    )
    
    val_ratio = config.VAL_RATIO / (config.TRAIN_RATIO + config.VAL_RATIO)
    X_train, X_val, y_reg_train, y_reg_val = train_test_split(
        X_temp, y_reg_temp, test_size=val_ratio, random_state=config.RANDOM_SEED
    )
    ...
    metrics = trainer.train(X_train, y_reg_train, X_val, y_reg_val, feature_names)
    
    # 6. 测试模型
    tester = LinearTester(trainer.model, config)
    target_stats = dataset.get_target_stats()
    test_results = tester.test(X_test, y_reg_test, target_stats['mean'], target_stats['std'], 
    ...
    if trainer.summary is not None:
        summary_df = pd.DataFrame({
            ...
            'p_value': trainer.summary['p_values'],
            'ci_lower': trainer.summary['ci_lower'],
            'ci_upper': trainer.summary['ci_upper'],
            'significant': trainer.summary['p_values'] < config.SIGNIFICANCE_LEVEL
        })
    ...
    # ✅ 新增：测试集 y_true vs y_pred 可视化
    visualizer.generate_all_test_plots(symbol, test_results)
    ...
    print(f"     显著因子数：{summary['n_significant']}")
...
def generate_summary_report(summaries: list, config: Config):
    ...
    if 'test_ic' in summary_df.columns:
        ...
        for _, row in top_ic.iterrows():
            print(f"   {row['symbol']}: IC={row['test_ic']:.4f}, R²={row['val_r2']:.4f}, 显著因子={row['n_significant']}")
    
    if 'val_r2' in summary_df.columns:
        top_r2 = summary_df.nlargest(5, 'val_r2')
        print("\n🏆 Top 5 交易对 (按验证 R²):")
        for _, row in top_r2.iterrows():
            print(f"   {row['symbol']}: R²={row['val_r2']:.4f}, IC={row['test_ic']:.4f}, 显著因子={row['n_significant']}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Linear Regression 因子训练测试脚本')
    ...
    print("="*60)
    print("🚀 Linear Regression 因子训练测试脚本 (增强可视化版)")
    print("="*60)
